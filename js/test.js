  ...
  function plotData(map) {
    ...
    svgScatterPlot.selectAll('.dot')
      ...
      .append('circle')
        ...
        .attr('cy', yMap)
        .attr('r', function(d) {
            return pop_map_func(d["pop_mlns"])})
        .attr('fill', "#4286f4")
        ...
        svgScatterPlot.selectAll('.dot').data(allYearsData.filter(function(d) {return d.time != selected})).attr("display", "none")
        // .attr('display', "none")
    svgScatterPlot.selectAll('.dot').data(data.filter(function(d) {if (time == selected) {
        console.log('YES')
    }})).enter().append("circle")
    .attr('display', "inline")
